 ...
 *
 * bkmk cannot fail that way today: its four faces come through
 * `next/font/google`, which self-hosts them at build time, so nothing the
 * document asks for is third-party — its CSP allows no stylesheet origin but
 * its own. The check is here because the six harnesses are copies of each
 * other, and whoever adds the first `<link>` should not have to find this a
 * second time. On a fully self-hosted app it costs a few milliseconds and never
 ...
 * a no-op in a production build, a floating logo in a dev one. Nothing of the app's own is listed:
 * bkmk confirms a write by navigating (a commit lands on the index, a save closes the modal), so
 * there is no toast viewport to decide about.
 */
...
export class Demo {
   ...
   * the shared fixture, kept here so the copies stay one file. Its donut and its
   * bars hit-test the cursor from ONE handler on the graphic, so an arc has no
   * box `moveTo` could aim at and the storyboard sends the hand to a point it
   * works out from the geometry. bkmk's load chart draws one element per bar,
   * so its storyboard has nothing to aim this at today.
   */
...
export const test = base.extend<{ demo: Demo }>({
  demo: async ({ page }, use) => {
    ...
    await page.close();
    if (video) await video.saveAs(join(OUT_DIR, "bkmk-demo.webm"));
